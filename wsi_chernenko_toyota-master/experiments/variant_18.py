...
The pipeline system performs the 18th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- Clustering: Spectral clustering with sklearn.cluster (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.SpectralClustering.html#sklearn.cluster.SpectralClustering) with default parameters. 
...
average Rand Index = 0.6064141414141414

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = -0.03913753310187143

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.5846050566852695

================ Statistics: ====================================
============ average number of created clusters: 7.25
============ average cluster size: 14.375
...
# Cluster sentences in every topic with Spectral Clustering (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.SpectralClustering.html#sklearn.cluster.SpectralClustering) and create an output file:
def cluster(compos_vectors, output):
    ...
    for value in compos_vectors.values():
        ...
        spectral = SpectralClustering().fit(all_for_topic)
        for i in range(len(spectral.labels_)):
            one_line = str(value[0][0].split(".")[0])+"."+str(spectral.labels_[i]) + "	" + (str(value[i][0])+"\n")
...
if __name__ == "__main__":
    ...
    clusters = cluster(compos_vectors, path+"/output_18.txt", 7)
    print("Done.")
