...
The pipeline system performs the 14th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- For pre-rpocessing: tokenization + remove punctuation + POS-Tagging (ignored, if no match with sense2vec model)
...
average Rand Index = 0.4344444444444444

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = 0.06929214483604185

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.24069765063661844
            ...
            for sent in paragr[1:]: #sent in a snippet
                ...
                for word in sent: #word
                    try:
                        ...
                        try:
                            freq, query_vector = model[word.split("|")[0]] #get a vector for the word
                            vector_sent.append(query_vector) #add a word-vector to a list for sentences in a snippet - BOW for all words in a snippet - now for a sentence - for every sentence
                        except:
                            continue
...
if __name__ == "__main__":
    ...
    clusters = cluster(compos_vectors, path+"/output_16.txt", 7)
    print("Done.")
