...
The pipeline system performs the 19th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- Clustering: Aglomerative Clustering with sklearn.cluster (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html#sklearn.cluster.AgglomerativeClustering) with the following parameters:
 	n_clusters = 7
...
average F1 = 0.6709946881062718

=========== Final average value of Rand Index: =============
average Rand Index = 0.41919191919191917

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = 0.030965983228799003

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.21695199743616783
...
# Cluster sentences in every topic with Aglomerative Clustering (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html#sklearn.cluster.AgglomerativeClustering) and create an output file:
def cluster(compos_vectors, output, number_clusters):
    ...
    for value in compos_vectors.values():
        ...
        aglomer = AgglomerativeClustering(n_clusters=7).fit(all_for_topic)
        for i in range(len(aglomer.labels_)):
            one_line = str(value[0][0].split(".")[0])+"."+str(aglomer.labels_[i]) + "	" + (str(value[i][0])+"\n")
...
if __name__ == "__main__":
    ...
    clusters = cluster(compos_vectors, path+"/output_19.txt", 7)
    print("Done.")
