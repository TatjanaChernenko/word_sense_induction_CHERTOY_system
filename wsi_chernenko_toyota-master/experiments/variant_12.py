...
The pipeline system performs the 12th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- Compositional semantics: tuned vector mixture model (BOW (bag-of-words) representation with summarization for each snippet) with alfa = 0.5 for non-topic words and alfa = 1.0 for topic words.
...
average Rand Index = 0.39575757575757575

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = 0.022343464372420508

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.18175332257231225
...
def compos_sense2vec(prepr_data,len_vector, topics_data):
    ...
    for line in topics_data:
        if not line.startswith("ID"):
            topic = line.split("	")[1]
            try:
                clean_topic = topic.split("_")
            except:
                clean_topic = topic 
            for i in range(len(clean_topic)):
                clean_topic[i] = clean_topic[i].strip()
            topics.append(clean_topic)
    for el in topics:  
        for w in el: 
            str_topics+=w
            str_topics+=" "
    str_topics = str_topics.split(" ")
            ...
            for sent in paragr[1:]:
                ...
                for word in sent:
                    ...
                    try:
                        ...
                        #print("a ", alfa) 
                        #print("qa ", query_vector*alfa)
                        vector_sent.append(query_vector*alfa)
                    except:
                        continue
...
if __name__ == "__main__":
    ...
    compos_vectors = compos_sense2vec(prepr_data, 128, topics_data)
    clusters = cluster(compos_vectors, path+"/output_12.txt", 7)
    print("Done.")
