...
The pipeline system performs the 16th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- Clustering: Affinity Propagation (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.AffinityPropagation.html#sklearn.cluster.AffinityPropagation) clustering with sklearn.cluster (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html) with default parameters. 

------------------- EVALUATION -------------------
=========== Final average value of F1: =====================
average F1 = 0.6973593001744707

=========== Final average value of Rand Index: =============
average Rand Index = 0.3690909090909091

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = 0.029939655995035758

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.1313309739934938

================ Statistics: ====================================
============ average number of created clusters: 14.25
============ average cluster size: 7.075320512820514
...
# Cluster sentences in every topic with Affinity Propagation (http://scikit-learn.org/stable/modules/generated/sklearn.cluster.AffinityPropagation.html#sklearn.cluster.AffinityPropagation) and create an output file:
def cluster(compos_vectors, output):
    f = open(output, "a")
    ...
    for value in compos_vectors.values():
        ...
        affinity = AffinityPropagation().fit(all_for_topic)
        for i in range(len(affinity.labels_)):
            one_line = str(value[0][0].split(".")[0])+"."+str(affinity.labels_[i]) + "	" + (str(value[i][0])+"\n")
...
if __name__ == "__main__":
    ...
    clusters = cluster(compos_vectors, path+"/output_16.txt")
    print("Done.")
