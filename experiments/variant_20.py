...
The pipeline system performs the 20th variant of the system for WSI (word sense induction) task (the Task 11 at SemEval 2013).
...
- Language model: sent2vec (https://github.com/epfml/sent2vec) with sent2vec_toronto books_unigrams model
...
average F1 = 0.6683161320632384

=========== Final average value of Rand Index: =============
average Rand Index = 0.4790909090909091

=========== Final average value of Adjusted Rand Index: ====
average Adj Rand Index = 0.07273862072387383

=========== Final average value of Jaccard Index: ==========
average Jaccard Index = 0.3163804742952381
...
# For every word in a sentence make a vector representation with sent2vec (https://github.com/epfml/sent2vec) with sent2vec_toronto books_unigrams model; make a compositional vector for every sentence as sum of BOW vectors:
def compos_sent2vec(prepr_data,len_vector):
    model = sent2vec.Sent2vecModel()
    model.load_model("/home/tatiana/MyPython/Other/sent2vec/sent2vec-master/torontobooks_unigrams.bin")
        ...
        for paragr in value: #one snippet
            ...
            for sent in paragr[1:]: #sent in a snippet
                ...
                for el in sent:
                    sen+=el+" "
                try:
                    query_vector = model.embed_sentence(sen)
                    #print("erfolg", query_vector)
                        #freq, query_vector = model[word] #get a vector for the word
                    par_list.append(query_vector) #add a word-vector to a list for sentences in a snippet - BOW for all words in a snippet - now for a sentence - for every sentence
                except:
                    continue
                #summe = np.zeros(len_vector) # vector for a summ 
                #for vector in vector_sent: # for one word in all sentences
                #summe+=query_vector # summ all words in a snippet - vector for a snippet
...
if __name__ == "__main__":
    ...
    clusters = cluster(compos_vectors, path+"/output_20.txt", 7)
    print("Done.")
